...
class HTTPClient:
    ...
    async def request(self, method, url, authorize=True,**kwargs):
        ...
        async with ClientSession(loop=self.loop) as session:
            async with session.request(method, url, **kwargs) as response:
                log.debug(f'{method} {url} has returned {response.status}')
    ...
    async def getBots(self, page:int = 1):
        Data = await self.request('GET', f'/bots/get', authorize=False, params={'page':page})

        return [Bot(_) for _ in Data.get('data', [])]

    async def searchBots(self, query:str, page:int = 1):
        Data = await self.request('GET', f'/bots/search', authorize=False, params={'q':query, 'page':page})
